...
class sa{constructor(a){this.g=a;this.position={x:0,y:0,z:0}}u(){this.g.element.style.transform=`scale(${D}) translate3d(${-this.position.x*window.innerHeight}px, ${this.position.y*window.innerHeight}px, ${-this.position.z*window.innerHeight}px)`}}function ta(){var a=E;a.element.style.left=.5*window.innerWidth+"px";a.element.style.top=.5*window.innerHeight+"px";a.o.u();a.g.forEach(b=>b.u())}
class ua{constructor(){this.o=new sa(this);this.g=new Set;this.element=document.createElement("div");this.i=document.createElement("div");this.i.appendChild(this.element);document.body.appendChild(this.i);this.element.id="scene";this.i.id="scene-parent"}add(a){this.g.add(a);this.element.appendChild(a.element)}remove(a){this.g.delete(a);this.element.removeChild(a.element)}}
...
La.addEventListener("mousedown",()=>{const a=new KeyboardEvent("keydown",{key:" ",code:"Space",keyCode:32,which:32,bubbles:!0,cancelable:!0});window.dispatchEvent(a)});La.addEventListener("mouseup",()=>{const a=new KeyboardEvent("keyup",{key:" ",code:"Space",keyCode:32,which:32,bubbles:!0,cancelable:!0});window.dispatchEvent(a)});let Ea=document.createElement("div");Ea.style.borderRadius="50%";Ea.style.background="radial-gradient(#ff6, #f52 30%, #0006 50%, #0000 80%)";
...
H.g.uniform1i(H.i.numBlackHoles,6);let M=[],N=[],Oa,Pa,O=(null!=(Pa=+window.localStorage.getItem("hit-the-space-progress"))?Pa:0)%(K.length-1);Ka.appendChild(document.createTextNode(O));for(let a=1;a<K.length;++a){K[a].h[0]+=K[a-1].h[0];K[a].h[1]+=K[a-1].h[1];for(let b of K[a].m)b[0]+=K[a].h[0],b[1]+=K[a].h[1];for(let b of K[a].j)b[0]+=K[a].h[0],b[1]+=K[a].h[1]}
function Qa(a){function b(e,m){let k,h=new Ha(e[2],null!=(k=e[3])?k:80,100*c(),m?Na:[d(),d(),d(),d()],.6*c()+.6,m,a);(h.C=m&&a<=O)&&(h.element.style.filter="hue-rotate(88deg)");h.position.x=e[0];h.position.y=e[1];E.add(h);return h}Ra(a-3);if(!(a>=K.length||0>a)){var c=da(a),d=()=>[.2*c(),c(),.5*c()+.5],f=0;M=[...M,Oa=b(K[a].h,!0),...K[a].m.map(e=>b(e,!1))];N=[...K[a].j.map(function(e){let m=a%3*2+f++,k=e[0],h=e[1];ya(m,k,h,e[2],e[3]);return[k,h,e[2],a,m]}),...N]}}Qa(O);Qa(O-1);Qa(O+1);
function Ra(a){0>a||(M=M.filter(b=>{let c=b.level===a;c&&E.remove(b);return!c}),N=N.filter(b=>{let c=b[3]===a;c&&ya(b[4],9E9,9E9,0,0);return!c}))}let P=new xa;E.add(P);P.position.x=0;P.position.y=0;let Q=M[0],Sa=0,Ta=0,Ua=0,ba=0,R=new wa;E.add(R);let S=2,r=0,t=0,T=Math.atan2(t,r),U=G.position.x,Va=G.position.y,V=K[O].l,W=V,X=!1,Wa=document.getElementById("overlay"),Xa=document.getElementById("boost-bar").style,Ya=document.getElementById("boost-container").style;Ya.width=10*V+"vh";
function Za(a){W=Math.max(W-a,0);Xa.width=W/V*100+"%"}function $a(){R.element.style.opacity=1;R.position.x=Q.position.x-Q.s-.05;R.position.y=Q.position.y;R.o=1;G.position.x=U=R.position.x;G.position.y=Va=R.position.y;t=r=0;T=Math.atan2(t,r);R.g.z=180*T/Math.PI;D=1;Wa.classList.remove("visible");Sa=1;S=2;Y=0;W=V;Za(0)}let ab=!1;window.StartGame=function(){ab||(ab=!0,ka(),$a(),bb=!1)};window.ResetGame=function(){window.localStorage.setItem("hit-the-space-progress",0);Ka.textContent=0};let cb=[];
function db(a,b){cb.push([a,b,!1])}function eb(a,b){cb.push([a,b,!0])}let fb=0,gb=0;const Z=1/60;
function hb(a){a/=1E3;var b=a-fb;fb=a;b=Math.min(b,.1);gb+=b;window.requestAnimationFrame(hb);for(a=X;gb>-Z/2;){gb-=Z;ib();for(b=0;b<cb.length;++b){var c=cb[b];c[2]&&c[0]();0>=(c[1]-=Z)&&(cb.splice(b,1),--b,c[0]())}Ja=Ia=!1}1===S&&(X=!1);a!==X&&(z.gain.linearRampToValueAtTime(X?0:1,u.currentTime),z.gain.linearRampToValueAtTime(X?1:0,u.currentTime+.1));a=.45/D;b=window.innerWidth/window.innerHeight*a;c=Oa.position.x-G.position.x;const d=Oa.position.y-G.position.y,f=l(d,-a,a);P.position.x=G.position.x+
l(c,-b,b);P.position.y=G.position.y+f;P.g.z=180*Math.atan2(d,c)/Math.PI;P.element.style.opacity=+(Math.abs(c)>1.5*b||Math.abs(d)>1.5*a);ta()}function jb(a){a>Math.PI?a-=2*Math.PI:a<-Math.PI&&(a+=2*Math.PI);return a}
function kb(){Ca(Ma,50,{x:R.position.x,y:R.position.y});B(50,.5,u.currentTime,.1,.5,.5,"square");B(70,.5,u.currentTime,.1,.3,.3,"sawtooth");C(.3,u.currentTime,A(),!0,1E3,2,.02,.6,0);S=1;Wa.classList.add("visible");R.element.style.opacity=0;y.gain.linearRampToValueAtTime(0,u.currentTime);y.gain.linearRampToValueAtTime(-20,u.currentTime+.5);db(()=>{$a();y.gain.linearRampToValueAtTime(-20,u.currentTime);y.gain.linearRampToValueAtTime(0,u.currentTime+.5)},1)}let bb=!0,Y=0;
function ib(){Ma.update(Z);if(!bb&&1!==S){var a=.02*Z,b=l(Math.abs(U-R.position.x),0,5)/5,c=l(Math.abs(Va-R.position.y),0,5)/5;U=g(U,R.position.x,g(b**1.5,b,Y));b=Va=g(Va,R.position.y,g(c**1.5,c,Y));c=2*R.position.y-Va;G.position.x=g(U,2*R.position.x-U,Y*Y*(3-2*Y));G.position.y=g(b,c,Y*Y*(3-2*Y));D=g(D,1/(1+.5*ca()),.5*Z);if(0===S){if(Ja){kb();return}for(var d of M){b=d.position.x-R.position.x;c=d.position.y-R.position.y;var f=b*b+c*c;let e=d.L/f;if(Math.sqrt(f)<d.s+R.s+(d.J&&!d.C?.05:0)){if(d.J&&
!d.C){S=2;r=t=0;Qa(++O+1);window.localStorage.setItem("hit-the-space-progress",O);Ka.textContent=O;B(555,.5,u.currentTime,.1,.2,10,"sawtooth");B(66,.5,u.currentTime,.1,.2,10,"sawtooth");X=!1;W=V=K[O].l;Ya.width=10*V+"vh";Za(0);Q=d;d.C=!0;d.element.style.filter="hue-rotate(88deg)";Sa=0;Ta=R.position.x;Ua=R.position.y;ba=T;eb(()=>{Y=l(Y-Z/2,0,1)},2);O===K.length-1&&(bb=!0,db(()=>{document.getElementById("start-text").style.display="none";document.getElementById("game-completed").style.display="";Wa.classList.add("visible")},
.5),v.gain.linearRampToValueAtTime(ea,u.currentTime),v.gain.linearRampToValueAtTime(0,u.currentTime+4),w=ea=0);return}kb();return}r+=b*e*a;t+=c*e*a}for(const e of N){d=e[0]-R.position.x;b=e[1]-R.position.y;c=d*d+b*b;f=2E3*e[2]/c;if(Math.sqrt(c)<e[2]+R.s){S=1;Wa.classList.add("visible");let m=0,k=R.position.x,h=R.position.y;eb(()=>{m+=Z/.3;R.element.style.opacity=R.o=Math.max(0,1-m);R.position.x=g(k,e[0],m);R.position.y=g(h,e[1],m)},.3);y.gain.linearRampToValueAtTime(0,u.currentTime);y.gain.linearRampToValueAtTime(-20,
u.currentTime+.5);db(()=>{$a();y.gain.linearRampToValueAtTime(-20,u.currentTime);y.gain.linearRampToValueAtTime(0,u.currentTime+.5)},1);return}r+=d*f*a;t+=b*f*a}if(X=L&&0<W)d=Math.cos(T),b=Math.sin(T),c=g(500,15,l(Math.hypot(r,t)/10,0,1)),r+=d*Z*c,t+=b*Z*c,c=.01*Math.random()+.025,Ca(Ma,1,{angle:T+(Math.random()-.5),x:R.position.x-.02*d,y:R.position.y-.02*b,speed:5*Z,F:c,G:c}),Za(Z);r*=1-5*a;t*=1-5*a;d=Math.hypot(r,t);b=Math.min(25,d);r*=b/d;t*=b/d;R.position.x+=r*a;R.position.y+=t*a;d=jb(Math.atan2(t,
r)-T);a*=80;d=l(d,-a,a);T=jb(T+d)}else 2===S&&(a=Math.atan2(R.position.y-Q.position.y,R.position.x-Q.position.x)+.15*Z/Q.s,b=Math.cos(a),d=Math.sin(a),Sa=Math.min(Sa+Z/.5,1),a+=Math.PI/2,b=Q.position.x+(Q.s+.05)*b,d=Q.position.y+(Q.s+.05)*d,c=1-(1-Sa)**2,T=aa(a,c),R.position.x=g(Ta,b,c),R.position.y=g(Ua,d,c),Ia&&(S=0,r=15*Math.cos(a),t=15*Math.sin(a),eb(()=>{Y=l(Y+Z/3,0,1)},3)));R.g.z=180*T/Math.PI}}window.requestAnimationFrame(hb);window.addEventListener("resize",()=>ta());
